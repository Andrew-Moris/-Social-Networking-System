...
document.addEventListener('DOMContentLoaded', function() {
    console.log('تم تحميل نظام المحادثة المصلح');
    ...
    function loadMessages() {
        ...
        xhr.open('GET', `api/chat.php?action=get_messages&user_id=${currentChatUserId}&last_id=${lastMessageId}&t=${timestamp}`, true);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    console.error('خطأ في الاتصال:', xhr.status);
                }
    ...
    function deleteMessage(messageId) {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'api/chat.php?action=delete_message', true);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                try {
                    ...
                    console.error('خطأ في تحليل استجابة الحذف:', e);
                }
            }
        };
        xhr.send(JSON.stringify({
