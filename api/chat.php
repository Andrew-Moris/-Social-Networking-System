    ...
    switch ($action) {
        case 'send_message':
            ...
            if (strpos($contentType, 'application/json') !== false) {
                ...
                $image_url = isset($data['media_url']) ? trim($data['media_url']) : null;
            } else {
            ...
            try {
                ...
                $stmt = $pdo->prepare("UPDATE users SET last_active = NOW(), last_message_time = ? WHERE id = ?");
                $stmt->execute([$message_time, $current_user_id]);
                
                $stmt = $pdo->prepare("UPDATE users SET last_message_time = ? WHERE id = ?");
                $stmt->execute([$message_time, $receiver_id]);
