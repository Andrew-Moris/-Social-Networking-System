...
if (empty($_FILES['media']) || !isset($_FILES['media']['name']) || empty($_FILES['media']['name'])) {
    jsonResponse(['success' => false, 'message' => 'لم يتم تحديد ملف للرفع']);
}

error_log("Post upload request received. File info: " . print_r($_FILES['media'], true));
...
if ($_FILES['media']['error'] !== UPLOAD_ERR_OK) {
    ...
    jsonResponse(['success' => false, 'message' => $error_message]);
}
...
if (!in_array($file_type, $allowed_types) && !in_array($file_extension, $allowed_extensions)) {
    ...
    jsonResponse(['success' => false, 'message' => 'نوع الملف غير مدعوم. الأنواع المدعومة هي: JPEG, PNG, GIF, WebP, MP4, WebM']);
}

$max_size = 10 * 1024 * 1024;
if ($_FILES['media']['size'] > $max_size) {
    jsonResponse(['success' => false, 'message' => 'حجم الملف كبير جدًا. الحد الأقصى هو 10 ميجابايت']);
}
...
try {
    ...
    if (!file_exists($upload_dir)) {
        if (!mkdir($upload_dir, 0777, true)) {
            ...
            jsonResponse(['success' => false, 'message' => 'فشل إنشاء مجلد الرفع']);
        }
    ...
    if (move_uploaded_file($_FILES['media']['tmp_name'], $file_path)) {
        ...
        if (copy($_FILES['media']['tmp_name'], $file_path)) {
            $upload_success = true;
            error_log("File uploaded successfully using copy");
        } else {
    ...
    if (!$upload_success) {
        jsonResponse(['success' => false, 'message' => 'فشل في نقل الملف المرفق. الرجاء المحاولة مرة أخرى.']);
    }
    
    if (!file_exists($file_path)) {
        error_log("File does not exist after upload: {$file_path}");
        jsonResponse(['success' => false, 'message' => 'فشل التحقق من وجود الملف بعد الرفع']);
    }
    ...
    error_log("Error in post_upload.php: " . $e->getMessage());
