...
echo "<html dir='rtl'><head><title>التحقق من قاعدة البيانات وإصلاحها</title>";
echo "<style>body { font-family: Arial, sans-serif; margin: 20px; } h1 { color: #2563eb; } 
.success { color: green; } .error { color: red; } .warning { color: orange; }
table { border-collapse: collapse; width: 100%; margin: 20px 0; }
table, th, td { border: 1px solid #ddd; }
th, td { padding: 12px; text-align: right; }
th { background-color: #f2f2f2; }
.btn { display: inline-block; padding: 10px 15px; background-color: #2563eb; color: white; 
text-decoration: none; border-radius: 4px; margin: 10px 0; }
</style></head><body>";

echo "<h1>التحقق من قاعدة البيانات وإصلاحها</h1>";
...
try {
    ...
    if (count($tables) === 0) {
        echo "<p class='warning'>جدول المستخدمين غير موجود. سيتم إنشاؤه الآن...</p>";
        ...
        echo "<p class='success'>جدول المستخدمين موجود</p>";
    }
    
    $users_count = $pdo->query("SELECT COUNT(*) as count FROM users")->fetch()['count'];
    echo "<p>عدد المستخدمين في قاعدة البيانات: <strong>{$users_count}</strong></p>";
    ...
    if (count($users) > 0) {
        ...
        echo "<p class='warning'>لا يوجد مستخدمين في قاعدة البيانات</p>";
    }
    ...
    if ($users_count < 2) {
        ...
        if (!isset($_SESSION['user_id'])) {
            echo "<p class='warning'>لا يوجد مستخدم حالي. سيتم إنشاء مستخدم جديد...</p>";
            
            $admin_check = $pdo->prepare("SELECT id FROM users WHERE username = ?");
            $admin_check->execute(['admin']);
            ...
            if (!$admin_user) {
                ...
                $insert_admin->execute(['admin', password_hash('admin123', PASSWORD_DEFAULT), 'admin@example.com', 'مدير', 'النظام', 'admin']);
                $admin_id = $pdo->lastInsertId();
                
                echo "<p class='success'>تم إنشاء المستخدم الرئيسي بنجاح. المعرف: {$admin_id}</p>";
                
                $_SESSION['user_id'] = $admin_id;
                $_SESSION['username'] = 'admin';
            } else {
                echo "<p class='success'>المستخدم الرئيسي موجود بالفعل. المعرف: {$admin_user['id']}</p>";
                
                $_SESSION['user_id'] = $admin_user['id'];
                $_SESSION['username'] = 'admin';
            }
        } else {
            echo "<p class='success'>المستخدم الحالي موجود. المعرف: {$_SESSION['user_id']}</p>";
        }
        
        $test_users = [
            ['test_user', 'test123', 'test@example.com', 'مستخدم', 'اختباري'],
        ...
        foreach ($test_users as $user_data) {
            ...
            if (!$test_user) {
                ...
                $insert_user->execute([
                    $user_data[0],
                    password_hash($user_data[1], PASSWORD_DEFAULT),
                    $user_data[2],
                    $user_data[3],
                    $user_data[4]
                ]);
                ...
                echo "<p class='success'>تم إنشاء المستخدم {$user_data[0]} بنجاح. المعرف: {$user_id}</p>";
            } else {
    ...
    if (count($tables) === 0) {
        echo "<p class='warning'>جدول الرسائل غير موجود. سيتم إنشاؤه الآن...</p>";
        ...
        echo "<p class='success'>تم إنشاء جدول الرسائل بنجاح</p>";
    } else {
        echo "<p class='success'>جدول الرسائل موجود</p>";
    }
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if ($other_user_data) {
            ...
            echo "<p>عدد الرسائل في قاعدة البيانات: <strong>{$messages_count}</strong></p>";
            ...
            if ($messages_count < 2) {
                ...
                foreach ($test_messages as $message_data) {
                    ...
                    echo "<p class='success'>تم إضافة رسالة اختبارية بنجاح. المعرف: {$message_id}</p>";
                }
            }
        }
    }
    ...
    echo "<a href='chat.php' class='btn' target='_blank'>فتح صفحة المحادثة</a> ";
    echo "<a href='login.php' class='btn' target='_blank'>تسجيل الدخول</a>";
    echo "</div>";
    
} catch (PDOException $e) {
    echo "<h1 class='error'>خطأ في الاتصال بقاعدة البيانات</h1>";
    echo "<p>{$e->getMessage()}</p>";
    
    try {
        $pdo_no_db = new PDO("mysql:host=$host", $user, $password, $options);
        echo "<p class='warning'>محاولة إنشاء قاعدة البيانات...</p>";
        
        $pdo_no_db->exec("CREATE DATABASE IF NOT EXISTS $dbname CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        echo "<p class='success'>تم إنشاء قاعدة البيانات بنجاح. يرجى تحديث الصفحة.</p>";
        
        echo "<script>setTimeout(function() { window.location.reload(); }, 3000);</script>";
    } catch (PDOException $e2) {
        echo "<p class='error'>فشل إنشاء قاعدة البيانات: {$e2->getMessage()}</p>";
    }
}
